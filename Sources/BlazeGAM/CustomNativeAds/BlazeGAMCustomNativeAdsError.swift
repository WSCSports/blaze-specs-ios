//
//  BlazeGAMCustomNativeAdsError.swift
//
//
...
/// `BlazeGAMCustomNativeAdsError` is a custom error type for handling specific ad-related errors in Google Ad Manager operations.
/// It conforms to both `Error` and `Equatable` to support error comparison and propagation.
public struct BlazeGAMCustomNativeAdsError: Error, Equatable {
...
/// It conforms to both `Error` and `Equatable` to support error comparison and propagation.
@available(*, unavailable, renamed: "BlazeGAMCustomNativeAdsError")
